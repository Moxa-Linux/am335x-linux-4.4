...
int tpm_tis_recv(struct tpm_chip *chip, u8 *buf, size_t count)
{
	...
	if (size < TPM_HEADER_SIZE) {
		dev_err(chip->dev.parent, "Unable to read header\n");
		goto out;
	}
	...
	if (size < expected) {
		dev_err(chip->dev.parent, "Unable to read remainder of result\n");
		size = -ETIME;
		goto out;
	}
	...
	if (status & TPM_STS_DATA_AVAIL) {	/* retry? */
		dev_err(chip->dev.parent, "Error left over data\n");
		size = -EIO;
		goto out;
	}
...
static int probe_itpm(struct tpm_chip *chip, bool *itpm)
{
	...
	*itpm = true;
	...
	rc = tpm_tis_send_data(chip, cmd_getticks, len, *itpm);
	if (rc == 0) {
		dev_info(chip->dev.parent, "Detected an iTPM.\n");
		rc = 1;
	} else {
		rc = -EFAULT;
	}
